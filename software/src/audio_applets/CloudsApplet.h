...
// CloudsApplet — Clouds-inspired live granular audio processor.
//
...
//   • Feedback/reverb — via Blend multi-mode (AuxButton cycles WD/FB/RV)
//   • No fixed grain shapes — shape is driven by Texture knob
//
// I/O:
//   Input → [AudioEffectClouds] → wet ──────────────────────────────┐
//   Input → [AudioEffectReverbSchroeder] (wet send) → reverb_out ──►│
//   Input →                          → dry ──────────────────────►  AudioMixer<3> → Output
//
// Blend multi-mode (AuxButton cycles):
//   WD  — Blend controls wet/dry ratio (equal-power crossfade)
//   FB  — Blend controls grain feedback amount
//   RV  — Blend controls reverb send (mix stays full wet)
//
// Parameters (two pages):
//   Page 1: Pos, Density, Size, Texture
//   Page 2: Pitch, Blend, (blend mode label), Mix
//
template <AudioChannels Channels>
class CloudsApplet : public HemisphereAudioApplet {
public:
    const char* applet_name() override { return "Clouds"; }
    ...
    void Controller() override {
        ...
        // density UI 0–100: map to −20..+20 Hz (50 = silence, ±20 Hz at extremes).
        // Negative = regular periodic; positive = stochastic cloud.
        ...
        // Blend param: effective value 0..1 from UI + CV.
        ...
        // Wet/dry gains depend on blend mode.
        ...
        switch (blend_mode_) {
            case BLEND_WD: {
                // Blend controls wet/dry balance.
            ...
            case BLEND_RV: {
                // Blend drives reverb send; grain runs full wet.
        ...
        for (int ch = 0; ch < Channels; ch++) {
            ...
            channels[ch].mixer.gain(CloudsChannel::DRY_CH,   dry_gain);
            channels[ch].mixer.gain(CloudsChannel::WET_CH,   wet_gain);
            channels[ch].mixer.gain(CloudsChannel::VERB_CH,  reverb_gain);
        }
    }
    ...
    void View() override {
        ...
        // ── Two pages, 4 rows each (y=15/25/35/45) ───────────────────────────
        ...
        if (!pg2) {
            ...
            // Density: display in Hz relative to centre (50=silence).
            // 0.4 * (density-50) → ±20 Hz range.
            gfxPrint(1, 25, "Den:");
            gfxStartCursor();
            int8_t d_hz = (int8_t)(0.4f * (density - 50)); // −20..+20 Hz
            ...
            // Blend: label changes with mode.
            static const char* BLEND_LABELS[] = { "WD:", "FB:", "RV:" };
            gfxPrint(1, 25, BLEND_LABELS[blend_mode_]);
            ...
            // Blend mode indicator at bottom, inverted when not WD to make it obvious.
            static const char* MODE_LABELS[] = { "WD", "FB", "RV" };
            gfxPrint(1, 56, "Md:");
            gfxPrint(MODE_LABELS[blend_mode_]);
            if (blend_mode_ != BLEND_WD) gfxInvert(22, 56, 14, 8);

            // Freeze latch indicator.
            if (manual_freeze_) gfxInvert(40, 56, 24, 8);
            gfxPrint(42, 56, "FRZ");
        }
    ...
    // AuxButton: cycle blend mode (WD → FB → RV → WD).
    void AuxButton() override {
        blend_mode_ = (BlendMode)((blend_mode_ + 1) % 3);
        CancelEdit();
    }
...
#define CLOUDS_PARAMS  pos, density, size, texture, pitch, psprd, blend, mix
    void OnDataRequest(std::array<uint64_t, CONFIG_SIZE>& data) override {
        data[0] = PackPackables(CLOUDS_PARAMS);
        data[1] = PackPackables(pos_cv, density_cv, size_cv, texture_cv);
        data[2] = PackPackables(pitch_cv, blend_cv, spray_cv, mix_cv);
        data[3] = PackPackables(freeze_input, (uint8_t)blend_mode_, spray);
    }

    void OnDataReceive(const std::array<uint64_t, CONFIG_SIZE>& data) override {
        UnpackPackables(data[0], CLOUDS_PARAMS);
        UnpackPackables(data[1], pos_cv, density_cv, size_cv, texture_cv);
        UnpackPackables(data[2], pitch_cv, blend_cv, spray_cv, mix_cv);
        uint8_t bm = 0;
...
private:
    ...
    int8_t  pos      = 50;  // 0–100%
    CVInputMap pos_cv;
    int8_t  density  = 75;  // 0–100 (50=silence; 0.4*(val-50) Hz, >50=stochastic cloud)
    ...
    bool      manual_freeze_ = false;  // not used (no latch in this applet — mode cycles instead)

    // Per-channel DSP struct.
    struct CloudsChannel {
